...
!
      ...
      open(unit=7,file='x.dat',form='formatted',status='unknown')
      do i = 1, 100
         x = (i-1) * 0.1
         call new_MATEL('X', io, io, [ x, 0.0_dp, 0.0_dp ], val, grad)
         write(7,"(a,2f10.4,2x,3f10.4)") "<s|X|s(x)>: ", x, val, grad
      enddo
      close(7)

      open(unit=7,file='s.dat',form='formatted',status='unknown')
      do i = 1, 100
         x = (i-1) * 0.1
         call new_MATEL('S', io, io, [ x, 0.0_dp, 0.0_dp ], val, grad)
         write(7,"(a,2f10.4,2x,3f10.4)") "<s|S|s(x)>: ", x, val, grad
      enddo
      close(7)

      open(unit=7,file='s-sp.dat',form='formatted',status='unknown')
      do i = 1, 100
         x = (i-1) * 0.1
         call new_MATEL('S', io, jpx, [ x, 0.0_dp, 0.0_dp ], val, grad)
         write(7,"(a,2f10.4,2x,3f10.4)") "<s|S|p(x)>: ", x, val, grad
      enddo
      close(7)
