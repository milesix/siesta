...
!
      module qmmm_neighbour
        ...
        real(dp), save :: rcoor = 0.0d0

        integer,  dimension(:), pointer, save, public :: mm_jan
        real(dp), dimension(:), pointer, save, public :: mm_r2ij
        real(dp), dimension(:,:), pointer, save, public :: mm_xij
        logical,  save       :: mm_pointers_allocated = .false.

        integer,  dimension(:), pointer, save, public :: qmmm_jan
        real(dp), dimension(:), pointer, save, public :: qmmm_r2ij
        real(dp), dimension(:,:), pointer, save, public :: qmmm_xij
        logical,  save       :: qmmm_pointers_allocated = .false.

        integer, save, public :: num_mmvecs
        integer, dimension(:), allocatable, save, public ::  
     .           grid_veclist
        integer, dimension(:,:), allocatable, save, public ::  grid_nr
        integer, dimension(:), allocatable, save, public ::  
     .           qmmm_veclist
        integer, dimension(:), allocatable, save, public ::  
     .           qmmm_veclistxat
        integer, dimension(:,:), allocatable, save, public ::  qmmm_nr

        public :: actualize_qmmm_mneighb, mm_mneighb, 
     .         qmmm_mneighb, sizeup_neighbour_arrays

      contains

      subroutine actualize_qmmm_mneighb(na_qm,na_mm,natot,noat,noaa,
     .             r,qmcell,pc,rcorteqmmm,rmaxo,actualiz,
     .             lattice_type,include_qmqm_vdw)
      ...
      real(dp) rcorteqmmm, rcorteqmmm2, rmaxo
      character lattice_type
      logical :: include_qmqm_vdw
      ...
      integer, dimension(:,:,:,:), allocatable :: pbc_condition
      integer :: grid_nrx_max, grid_nry_max, grid_nrz_max
      integer :: nx, ny, nz

c variables de la lista de vecinos
      integer dimvec, acs
      integer, dimension(:), allocatable ::  grid_veclistemp
      integer, dimension(:,:), allocatable ::  grid_nrtemp

      integer, dimension(:), allocatable ::  qmmm_veclistemp
      integer, dimension(:,:), allocatable ::  qmmm_nrtemp

      integer, dimension(3) :: grid_nr_indx
...
! For now, we put a range of orbital equals to rmax0 (bohrs)
         rcoor=(rcorteqmmm+2.0)/0.529177d0+rmaxo   
      endif  
...
C     Initialize routine for neighbour search. This call creates the cells
         ...
         if(associated(qmmm_jan)) deallocate(qmmm_jan)
         allocate(qmmm_jan(maxnna))
         if(associated(qmmm_r2ij)) deallocate(qmmm_r2ij)
         allocate(qmmm_r2ij(maxnna))
         if(associated(qmmm_xij)) deallocate(qmmm_xij)
         allocate(qmmm_xij(3,maxnna))

         if(allocated(grid_veclist)) deallocate(grid_veclist)
         allocate(grid_veclistemp(dimvec))
         if(allocated(grid_nr)) deallocate(grid_nr)
         allocate(grid_nrtemp(3,dimvec))
         if(allocated(qmmm_veclist)) deallocate(qmmm_veclist)
         allocate(qmmm_veclistemp(dimvec))
         if(allocated(qmmm_veclistxat)) deallocate(qmmm_veclistxat)
         allocate(qmmm_veclistxat(na_qm))
         if(allocated(qmmm_nr)) deallocate(qmmm_nr)
         allocate(qmmm_nrtemp(3,dimvec))
         ...
         grid_nrx_max=int(rcoor/sqrt(qmcell(1,1)*qmcell(1,1)
     .        +qmcell(2,1)*qmcell(2,1)+qmcell(3,1)*qmcell(3,1)))+1
         grid_nry_max=int(rcoor/sqrt(qmcell(1,2)*qmcell(1,2)
     .        +qmcell(2,2)*qmcell(2,2)+qmcell(3,2)*qmcell(3,2)))+1
         grid_nrz_max=int(rcoor/sqrt(qmcell(1,3)*qmcell(1,3)
     .        +qmcell(2,3)*qmcell(2,3)+qmcell(3,3)*qmcell(3,3)))+1

         allocate(pbc_condition(na_mm,-grid_nrx_max:grid_nrx_max,
     .           -grid_nry_max:grid_nry_max,-grid_nrz_max:grid_nrz_max))
...
C     Look for neighbours of atom ia
               ...
               do in=1,nna
                  j=qmmm_jan(in)
                  if (j<=na_qm) then
                     drij(1:3)=qmmm_xij(1:3,in)-r(1:3,j)+
     .                    r(1:3,i)
                     call get_pbc_vectors(lattice_type,qmcell,kcell,
     .                    drij,grid_nr_indx)
                     if (i==j) then
                        if(grid_nr_indx(1)==0) then
                           if(grid_nr_indx(2)==0) then
                              if(grid_nr_indx(3)==0) cycle
                           endif
                        endif
                     endif
                     qmmm_veclistemp(n)=j
                     ...
                     if(noaa(j-na_qm).eq.'HOH') then
                        if (noat(j-na_qm).ne.'O') cycle
                     endif
                     drij(1:3)=qmmm_xij(1:3,in)-r(1:3,j)+
     .                    r(1:3,i)
                     call get_pbc_vectors(lattice_type,qmcell,kcell,
     .                    drij,grid_nr_indx)
                     if(noaa(j-na_qm).eq.'HOH') then
                        ...
                        qmmm_nrtemp(3,n)=grid_nr_indx(3)
                        n=n+1
                        if(pc(j-na_qm).ne.0.0)
     .                       pbc_condition(j-na_qm,grid_nr_indx(1),
     .                       grid_nr_indx(2),grid_nr_indx(3))=1
                        qmmm_veclistemp(n)=j+1
                        qmmm_nrtemp(1,n)=grid_nr_indx(1)
                        qmmm_nrtemp(2,n)=grid_nr_indx(2)
                        qmmm_nrtemp(3,n)=grid_nr_indx(3)
                        n=n+1
                        if(pc(j+1-na_qm).ne.0.0)
     .                       pbc_condition(j+1-na_qm,grid_nr_indx(1),
     .                       grid_nr_indx(2),grid_nr_indx(3))=1
                        ...
                        qmmm_nrtemp(3,n)=grid_nr_indx(3)
                        n=n+1
                        if(pc(j+2-na_qm).ne.0.0)
     .                       pbc_condition(j+2-na_qm,grid_nr_indx(1),
     .                       grid_nr_indx(2),grid_nr_indx(3))=1
                     else if(noaa(j-na_qm).ne.'HOH') then
                        qmmm_veclistemp(n)=j
                        qmmm_nrtemp(1,n)=grid_nr_indx(1)
                        qmmm_nrtemp(2,n)=grid_nr_indx(2)
                        qmmm_nrtemp(3,n)=grid_nr_indx(3)
...
c     se fija si la dimension de veclist es suficiente
     ...
     .                 ,n-1,dimvec
                  write(6,*) 'actualize_qmmm_mneighb: '//
     .                 'Stopping Program'
                  STOP
               endif
            enddo
...
c     alocate a veclist posta
         allocate(qmmm_veclist(n-1))
         qmmm_veclist(1:n-1)=qmmm_veclistemp(1:n-1)
         deallocate(qmmm_veclistemp)
         allocate(qmmm_nr(3,n-1))
         qmmm_nr(1:3,1:n-1)=qmmm_nrtemp(1:3,1:n-1)
         deallocate(qmmm_nrtemp)
            ...
            do nx=-grid_nrx_max,grid_nrx_max
               do ny=-grid_nry_max,grid_nry_max
                  do nz=-grid_nrz_max,grid_nrz_max
                     if (pbc_condition(j,nx,ny,nz)==1) then
...
c     alocate a grid_veclist posta
         allocate(grid_veclist(n-1))
         grid_veclist(1:n-1)=grid_veclistemp(1:n-1)
         deallocate(grid_veclistemp)
         allocate(grid_nr(3,n-1))
         grid_nr(1:3,1:n-1)=grid_nrtemp(1:3,1:n-1)
         deallocate(grid_nrtemp)
         num_mmvecs=n-1

      endif

      end subroutine actualize_qmmm_mneighb

      subroutine mm_mneighb( CELL, range, NA, XA, IA, ISC,
     .                    NNA )
...
C                     only neighbours with JA.LE.IA are included in MM_JAN
!!!!! C INTEGER MAXNNA    : Size of arrays MM_JAN, MM_XIJ and MM_R2IJ
C *********** OUTPUT *************************************************
C INTEGER NNA       : Number of neighbour atoms within range of IA
C INTEGER MM_JAN(NNA)  : Atom-index of neighbours
C REAL*8  MM_XIJ(3,NNA): Vectors from atom IA to neighbours
C REAL*8  MM_R2IJ(NNA) : Squared distances to neighbours
C *********** UNITS **************************************************
...
C   interatomic vectors MM_XIJ and distances MM_R2IJ.
...
C   the arrays MM_JAN, MM_XIJ and MM_R2IJ, i.e. if NNAout > NNAin, these arrays
...
C    DIMENSION MM_JAN(MAXNNA), MM_XIJ(3,MAXNNA)
C    Define CELL and initial positions XA
C    DO ITER = 1,NITER                (Molecular dynamics iteration)
C      NNA = MAXNNA
C      CALL NEIGHB( CELL, range, NA, XA, 0, 1, NNA, MM_JAN, MM_XIJ, MM_R2IJ )
C      IF (NNA .GT. MAXNNA) STOP 'MAXNNA too small'
C      Initialize to zero all atomic forces FA(IX,IA)
C      DO IA = 1,NA                   (Loop on atoms)
C        NNA = MAXNNA
C        CALL NEIGHB( CELL, range, NA, XA, IA, 1, NNA, MM_JAN, MM_XIJ, MM_R2IJ )
C        IF (NNA .GT. MAXNNA) STOP 'MAXNNA too small'
C        DO IN = 1,NNA                (Loop on neighbours of IA)
C          JA = MM_JAN(IN)               (Atomic index of neighbour)
C          RIJ = SQRT(MM_R2IJ(IN))       (Interatomic distance)
C          IF (RIJ .GT. 1.D-12) THEN  (Discard atom itself)
C            Find interatomic force FIJ( RIJ )
C            DO IX = 1,3
C              FA(IX,IA) = FA(IX,IA) - FIJ * MM_XIJ(IX,IN) / RIJ
C              FA(IX,JA) = FA(IX,JA) + FIJ * MM_XIJ(IX,IN) / RIJ
C            ENDDO
C          ENDIF
C        ENDDO
C      ENDDO
...
!        call re_alloc(mm_jan,1,maxnna,name='jan')
!        call re_alloc(mm_r2ij,1,maxnna,name='r2ij')
!        call re_alloc(mm_xij,1,3,1,maxnna,name='xij')
...
C Notify to rangeR that CELL has changed
          call mm_mranger( 'CELL', NX, CELL, range, NA, XA,
     .                 NA, IAMOVE, IA, ISC, X0,
     .                 NNA, MAXNNA )
        endif

C Notify to rangeR that atoms have moved
        call mm_mranger( 'MOVE', NX, CELL, range, NA, XA,
     .               NA, IAMOVE, IA, ISC, X0,
     .               NNA, MAXNNA )
...
C Find neighbours of atom IA
      if (IA .GT. 0) 
     .  call mm_mranger( 'FIND', NX, CELL, range, NA, XA,
     .               NA, IAMOVE, IA, ISC, X0,
     .               NNA, MAXNNA )

      end subroutine mm_mneighb

      SUBROUTINE Mm_Mranger( MODE, NX, CELL, range, NA, XA,
...
C INTEGER MM_JAN(NNAin)   : Atom-index of neighbours (only for MODE='FIND')
C REAL*8  MM_XIJ(NX,NNAin): Vectors from atom IA0 or point X0 to neighbours
C                        in cartesian coordinates (only for MODE='FIND')
C REAL*8  MM_R2IJ(NNAin)  : Squared distances to neighbours
...
C    DIMENSION CELL(3,3),MM_JAN(MAXNNA),MM_R2IJ(MAXNNA),MM_XIJ(3,MAXNNA),XA(3,NA)
C    Define CELL and initial positions XA
C    DO ITER = 1,NITER                (Molecular dynamics iteration)
C      CALL rangeR('MOVE',3,CELL,range,NA,XA,NA,0,0,1,X0,
C                  NNA,MM_JAN,MM_XIJ,MM_R2IJ)
C      Initialize to zero all atomic forces FA(IX,IA)
C      DO IA = 1,NA                   (Loop on atoms)
C        NNA = MAXNNA
...
C    DIMENSION CELL(3,3),MM_JAN(MAXNNA),MM_R2IJ(MAXNNA),MM_XIJ(3,MAXNNA),
C              XA(3,NA),XNEW(3)
C    Define CELL and initial positions XA
...
C               Check that array arguments are not overflooded
                IF (NNA .GT. MAXNNA) THEN
                  MAXNNA = MAXNNA + NA
                  call re_alloc(mm_jan,1,maxnna,name='mm_jan',
     .              routine='mm_mranger')
                  call re_alloc(mm_r2ij,1,maxnna,name='mm_jan',
     .              routine='mm_mranger')
                  call re_alloc(mm_xij,1,nx,1,maxnna,name='mm_jan',
     .              routine='mm_mranger')
                ENDIF
                MM_JAN(NNA) = JA
                do IX = 1,NX
...
C This is the unique return point
      ...
      subroutine qmmm_mneighb( CELL, range, NA, XA, IA, ISC,
     .                    NNA )
...
C                     a first call with IA = 0
C INTEGER ISC       : Single-counting switch (0=No, 1=Yes). If ISC=1,
C                     only neighbours with JA.LE.IA are included in QMMM_JAN
!!!!! C INTEGER MAXNNA    : Size of arrays QMMM_JAN, QMMM_XIJ and QMMM_R2IJ
C *********** OUTPUT *************************************************
C INTEGER NNA       : Number of neighbour atoms within range of IA
C INTEGER QMMM_JAN(NNA)  : Atom-index of neighbours
C REAL*8  QMMM_XIJ(3,NNA): Vectors from atom IA to neighbours
C REAL*8  QMMM_R2IJ(NNA) : Squared distances to neighbours
C *********** UNITS **************************************************
C Units of CELL, range and XA are arbitrary but must be equal
C *********** SUBROUTINES USED ***************************************
C CHKDIM, DISMIN, RECLAT
...
C   be larger than the cell size, in which case many 'images' of the
C   same atom will be included in the neighbour list, with different
C   interatomic vectors QMMM_XIJ and distances QMMM_R2IJ.
C The atom IA itself is included in the neighbour list, with zero
C   distance. You have to discard it if you want so.
C If the number of neighbour atoms found is larger than the size of
C   the arrays QMMM_JAN, QMMM_XIJ and QMMM_R2IJ, i.e. if NNAout > NNAin, these arrays
C   are filled only up to their size NNAin. With dynamic memory
C   allocation, this allows to find first the required array sizes
C   and then find the neighbours. Notice however that no warning is
C   given, so that you should always check that NNAout.LE.NNAin.
C *********** USAGE **************************************************
C Sample usage for a molecular dynamics simulation:
C    DIMENSION QMMM_JAN(MAXNNA), QMMM_XIJ(3,MAXNNA)
C    Define CELL and initial positions XA
C    DO ITER = 1,NITER                (Molecular dynamics iteration)
C      NNA = MAXNNA
C      CALL NEIGHB( CELL, range, NA, XA, 0, 1, NNA, QMMM_JAN, QMMM_XIJ, QMMM_R2IJ )
C      IF (NNA .GT. MAXNNA) STOP 'MAXNNA too small'
C      Initialize to zero all atomic forces FA(IX,IA)
C      DO IA = 1,NA                   (Loop on atoms)
C        NNA = MAXNNA
C        CALL NEIGHB( CELL, range, NA, XA, IA, 1, NNA, QMMM_JAN, QMMM_XIJ, QMMM_R2IJ )
C        IF (NNA .GT. MAXNNA) STOP 'MAXNNA too small'
C        DO IN = 1,NNA                (Loop on neighbours of IA)
C          JA = QMMM_JAN(IN)               (Atomic index of neighbour)
C          RIJ = SQRT(QMMM_R2IJ(IN))       (Interatomic distance)
C          IF (RIJ .GT. 1.D-12) THEN  (Discard atom itself)
C            Find interatomic force FIJ( RIJ )
C            DO IX = 1,3
C              FA(IX,IA) = FA(IX,IA) - FIJ * QMMM_XIJ(IX,IN) / RIJ
C              FA(IX,JA) = FA(IX,JA) + FIJ * QMMM_XIJ(IX,IN) / RIJ
C            ENDDO
C          ENDIF
C        ENDDO
C      ENDDO
...
C Nullify pointers
      if (.not. qmmm_pointers_allocated) then
        ...
        allocate(qmmm_xij(3,maxnna))
      !  Dimension arrays to initial size MAXNNA
  !      call re_alloc(qmmm_jan,1,maxnna,name='jan')
  !      call re_alloc(qmmm_r2ij,1,maxnna,name='r2ij')
  !      call re_alloc(qmmm_xij,1,3,1,maxnna,name='xij')
...
C Notify to rangeR that CELL has changed
          call qmmm_mranger( 'CELL', NX, CELL, range, NA, XA,
     .                 NA, IAMOVE, IA, ISC, X0,
     .                 NNA, MAXNNA )
        endif

C Notify to rangeR that atoms have moved
        call qmmm_mranger( 'MOVE', NX, CELL, range, NA, XA,
     .               NA, IAMOVE, IA, ISC, X0,
     .               NNA, MAXNNA )
...
C Find neighbours of atom IA
      if (IA .GT. 0) 
     .  call qmmm_mranger( 'FIND', NX, CELL, range, NA, XA,
     .               NA, IAMOVE, IA, ISC, X0,
     .               NNA, MAXNNA )

      end subroutine qmmm_mneighb

      SUBROUTINE qmmm_Mranger( MODE, NX, CELL, range, NA, XA,
     .                    NAMOVE, IAMOVE, IA0, ISC, X0,
     .                    NNA, MAXNNA )
...
C INTEGER NNA          : Number of 'neighbour' atoms within range of
C                        atom IA0 or position X0 (only for MODE='FIND')
C INTEGER QMMM_JAN(NNAin)   : Atom-index of neighbours (only for MODE='FIND')
C REAL*8  QMMM_XIJ(NX,NNAin): Vectors from atom IA0 or point X0 to neighbours
C                        in cartesian coordinates (only for MODE='FIND')
C REAL*8  QMMM_R2IJ(NNAin)  : Squared distances to neighbours
C                        (only for MODE='FIND')
C *********** UNITS **************************************************
C Units of CELL, range, XA and X0 are arbitrary but must be equal.
C All vectors in cartesian coordinates.
...
C *********** USAGE **************************************************
C Example of usage for a molecular dynamics simulation:
C    DIMENSION CELL(3,3),QMMM_JAN(MAXNNA),QMMM_R2IJ(MAXNNA),QMMM_XIJ(3,MAXNNA),XA(3,NA)
C    Define CELL and initial positions XA
C    DO ITER = 1,NITER                (Molecular dynamics iteration)
C      CALL rangeR('MOVE',3,CELL,range,NA,XA,NA,0,0,1,X0,
C                  NNA,QMMM_JAN,QMMM_XIJ,QMMM_R2IJ)
C      Initialize to zero all atomic forces FA(IX,IA)
C      DO IA = 1,NA                   (Loop on atoms)
C        NNA = MAXNNA
C        CALL rangeR('FIND',3,CELL,range,NA,XA,0,0,IA,1,X0,
...
C
C Example of usage for a Montecarlo simulation:
C    DIMENSION CELL(3,3),QMMM_JAN(MAXNNA),QMMM_R2IJ(MAXNNA),QMMM_XIJ(3,MAXNNA),
C              XA(3,NA),XNEW(3)
C    Define CELL and initial positions XA
C    CALL rangeR('CELL',3,CELL,range,NA,XA,0,0,0,0,XNEW,
C                 NNA,QMMM_JAN,QMMM_XIJ,QMMM_R2IJ)
...
C               Check that array arguments are not overflooded
                IF (NNA .GT. MAXNNA) THEN
                  MAXNNA = MAXNNA + NA
                  call re_alloc(qmmm_jan,1,maxnna,name='qmmm_jan',
     .              routine='qmmm_mranger')
                  call re_alloc(qmmm_r2ij,1,maxnna,name='qmmm_jan',
     .              routine='qmmm_mranger')
                  call re_alloc(qmmm_xij,1,nx,1,maxnna,name='qmmm_jan',
     .              routine='qmmm_mranger')
                ENDIF
                QMMM_JAN(NNA) = JA
                do IX = 1,NX
                  QMMM_XIJ(IX,NNA) = DX(IX)
                enddo
                QMMM_R2IJ(NNA) = R2
              ENDIF
            ENDIF
