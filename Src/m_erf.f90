module m_erf
  !! Error function definitions. Same as in QE's `qe_erf.mod`.
...
contains
  ...
  function erf_func (x)
    ...
    if (abs (x) > 6.0_DP) then
       ...
       !
       erf_func = sign (1.0_DP, x)
    else
       if (abs (x)  <= 0.47_DP) then
          x2 = x**2
          erf_func=x *(p1 (1) + x2 * (p1 (2) + x2 * (p1 (3) + x2 * p1 (4) ) ) ) &
               / (q1 (1) + x2 * (q1 (2) + x2 * (q1 (3) + x2 * q1 (4) ) ) )
       else
          erf_func = 1.0_DP - erfc_func (x)
       endif
    endif
    !
    return
