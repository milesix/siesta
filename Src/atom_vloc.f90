...
module atom_vloc
  ...
  use radial, only : rad_func_t, rad_matching_radius, rad_get_radius_from_value,&
       rad_smooth_large, rad_smooth, rad_cutoff, rad_multiply_by_rl, rad_dealloc,&
       rad_sum_value, rad_set_origin
  ...
  use pseudopotential_new, only : pseudopotential_new_t, get_pseudo_valence_charge, &
       get_pseudo_down, pseudo_has_core_charge, get_vlocal, set_pseudo_vlocal
...
contains
  ...
  subroutine gen_vlocal(isp)
    ...
    type(rad_func_t) :: chlocal, reduced_vlocal,rad_tmp, vdown, vdown_lmaxkb, vlocal
    ...
    vdown_lmaxkb = get_pseudo_down(vps, lmxkb)

    r_match = 0.0_dp
    do i=0,lmxkb-1
       vdown   = get_pseudo_down(vps,i)
       r_i     = rad_matching_radius(vdown,vdown_lmaxkb)
    ...
    do i=0, lmxkb
       vdown   = get_pseudo_down(vps,i)
       r_2zval = rad_get_radius_from_value(vdown,match_value)
    ...
    if (r_2zval.gt.1.30_dp*r_match) then
       ...
       vdown = get_pseudo_down(vps,0)
       call rad_smooth_large(vdown,zval,r_match,chlocal,vlocal)
       ...
       vdown = get_pseudo_down(vps,0)
        call rad_smooth(vdown,zval,r_match,chlocal,vlocal)
    ...
    rad_tmp = rad_multiply_by_rl(vlocal,1)
    ...
    call set_pseudo_vlocal(vps,vlocal)
