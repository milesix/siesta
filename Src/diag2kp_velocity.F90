...
#ifdef MPI
subroutine diag2kp_velocity( spin, no_l, no_u, no_s, nnz, &
  ...
  call diag2kp_velocity_1d(no_u2, aux(1,1,3), eig_aux)
  call diag2kp_velocity_2d(3,no_u2, aux(1,1,4), SJ)
  ...
  call re_alloc( g_ncol, 1, no_u, name='g_ncol', routine= 'diag2kp_velocity' )
  call re_alloc( g_ptr, 1, no_u, name='g_ptr', routine= 'diag2kp_velocity' )
  call re_alloc( g_col, 1, g_nnz, name='g_col', routine= 'diag2kp_velocity' )
  call re_alloc( g_H, 1, g_nnz, 1, spin%H, name='g_H', routine= 'diag2kp_velocity' )
  call re_alloc( g_S, 1, g_nnz, name='g_S', routine= 'diag2kp_velocity' )
  call re_alloc( g_xij, 1, 3, 1, g_nnz, name='g_xij', routine= 'diag2kp_velocity' )
  call re_alloc( v, 1, 3, 1, no_u2, name='v', routine= 'diag2kp_velocity' )
  ...
  call re_alloc( g_EDM, 1, g_nnz, 1, spin%EDM, name='g_EDM', routine= 'diag2kp_velocity' )
...
!$OMP end parallel
  ...
  call de_alloc( g_DM, name='g_DM', routine= 'diag2kp_velocity' )
...
999 continue
  ...
  call de_alloc(vdeg, name='vdeg', routine= 'diag2kp_velocity' )
  call de_alloc(Identity, name='identity', routine= 'diag2kp_velocity' )

  ! Clean-up the velocities
  call de_alloc( v, name='v', routine= 'diag2kp_velocity' )

  ! Clean up memory
  call de_alloc( g_ncol, name='g_ncol', routine= 'diag2kp_velocity' )
  call de_alloc( g_ptr, name='g_ptr', routine= 'diag2kp_velocity' )
  call de_alloc( g_col, name='g_col', routine= 'diag2kp_velocity' )
  call de_alloc( g_H, name='g_H', routine= 'diag2kp_velocity' )
  call de_alloc( g_S, name='g_S', routine= 'diag2kp_velocity' )
  call de_alloc( g_xij, name='g_xij', routine= 'diag2kp_velocity' )

#ifdef DEBUG
  call write_debug( '    POS diag2kp_velocity' )
#endif
...
!$OMP parallel do default(shared), private(io,jo,ind,kxij,kph)
    do io = 1, no_u
      do ind = g_ptr(io) + 1, g_ptr(io) + g_ncol(io)
        ...
        dH(1,jo,1,io) = dH(1,jo,1,io) + g_H(ind,1) * kph
        dH(2,jo,2,io) = dH(2,jo,2,io) + g_H(ind,2) * kph
        dH(1,jo,2,io) = dH(1,jo,2,io) + cmplx(g_H(ind,3), -g_H(ind,4), dp) * kph
        dH(2,jo,1,io) = dH(2,jo,1,io) + cmplx(g_H(ind,3), g_H(ind,4), dp) * kph
...
!!$    end do
  ...
  subroutine diag2kp_velocity_1d(n,from,to)
    integer, intent(in) :: n
    real(dp), intent(in), target :: from(n)
    real(dp), pointer :: to(:)
    to => from(:)
  end subroutine diag2kp_velocity_1d

  subroutine diag2kp_velocity_2d(n1,n2,from,to)
    integer, intent(in) :: n1,n2
    real(dp), intent(in), target :: from(n1,n2)
    real(dp), pointer :: to(:,:)
    to => from(:,:)
  end subroutine diag2kp_velocity_2d

end subroutine diag2kp_velocity


#else

subroutine diag2kp_velocity_dummy()
end subroutine diag2kp_velocity_dummy
