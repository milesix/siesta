...
!   LDA+U projectors
    ...
    allocate( occupation( max_number_ldau_proj,   &
 &                        max_number_ldau_proj,   &
 &                        na_u,                   &
 &                        spin%Grid) )
    call memory( 'A', 'D', size(occupation), 'occ_proj_ldau' )
    allocate( occupation_old( max_number_ldau_proj,       &
 &                            max_number_ldau_proj,       &
 &                            na_u,                       &
 &                            spin%Grid) )
    call memory( 'A', 'D', size(occupation_old), 'occ_proj_ldau' )
...
! Initialize arrays Di and listed only once
 ...
 &               name='Di', routine='occ_proj_ldau' )
  Di = 0.0_dp

  call re_alloc( listed, 1, no_s,                           &
 &               name='listed', routine='occ_proj_ldau')
  listed = .false.
...
! &                  spin%dm  
!  write(6,'(a,i5)')                                         &
! &  'occ_proj_ldau: maximum number of LDA+U projectors = ', &
! &                  max_number_ldau_proj
!  write(6,'(a,3f12.5)')                                     &
! &  'occ_proj_ldau: rmaxo, rmaxldau, rmax = ' ,             &
! &                  rmaxo, rmaxldau, rmax 
!! End debugging
...
!         Check maxno - if too small then increase array sizes
     ...
     &                       copy=.true., routine='occ_proj_ldau' )
...
! Global reduction of occupation
  call re_alloc( buffer1, 1, max_number_ldau_proj, 1, max_number_ldau_proj,  &
 &               name='buffer1', routine = 'occ_proj_ldau')
  do ka = 1, na_u
    is = isa(ka)
    spp => species(is)
    nldauproj = spp%nprojsldau
    if( nldauproj == 0 ) cycle
    do ispin = 1, spin%grid
      call globalize_sum(occupation(1:nldauproj,1:nldauproj,ka,ispin),       &
 &                       buffer1(1:nldauproj,1:nldauproj))
      occupation(1:nldauproj,1:nldauproj,ka,ispin) =                         &
 &         buffer1(1:nldauproj,1:nldauproj)
    enddo
  enddo
  call de_alloc(buffer1, name='buffer1', routine = 'occ_proj_ldau')
#endif
...
! &          'occ_proj_ldau : Node, Nodes, ka, mproj, mprojprime, spin, occ =',&
! &                           Node, Nodes, ka, mproj, mprojprime, ispin,       &
! &                           occupation(mproj,mprojprime,ka,ispin)
!        enddo 
!      enddo 
!    enddo
!    write(6,'(a,3i5,2f12.5)')' occ_proj_ldau: Node, Nodes, ka, traceupup = ' , &
! &    Node, Nodes, ka, traceupup(ka)
!    write(6,'(a,3i5,2f12.5)')' occ_proj_ldau: Node, Nodes, ka, tracedndn = ' , &
! &    Node, Nodes, ka, tracedndn(ka)
!    write(6,'(a,3i5,2f12.5)')' occ_proj_ldau: Node, Nodes, ka, traceupdn = ' , &
! &    Node, Nodes, ka, traceupdn(ka)
!    write(6,'(a,3i5,2f12.5)')' occ_proj_ldau: Node, Nodes, ka, tracednup = ' , &
! &    Node, Nodes, ka, tracednup(ka)
!    write(6,'(a,3i5,2f12.5)')' occ_proj_ldau: Node, Nodes, ka, tracetot  = ' , &
! &    Node, Nodes, ka, tracetot(ka)
!    write(6,'(a,3i5,f12.5)')' occ_proj_ldau: Node, Nodes, ka, number_cor= ' , &
! &    Node, Nodes, ka, number_corr_electrons(ka)
!    write(6,'(a,3i5,3f12.5)')' occ_proj_ldau: Node, Nodes, ka, magnetic  = ' , &
...
! Stop time counter
  call timer( 'occ_proj_ldau', 2 )

end subroutine occ_proj_ldau
