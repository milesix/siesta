...
subroutine ldau_so_hamil_2( H_ldau_so, fal, stressl )
  ...
  use atmfuncs,      only : orb_gindex   ! Returns the global index of a 
                                         !   basis orbital
  use atmfuncs,      only : ldau_gindex  ! Returns the global index of a 
                                         !   LDA+U projector
  ...
  use m_new_matel,     only : new_matel  ! Returns the overlap matrix elements
                                         ...
                                         !   LDA+U projector 
  use m_energies,      only : E_ldau_so  ! Contribution to the energy coming
  ...
  complex(dp), intent(inout) :: H_ldau_so(maxnh,spin%Grid)
  ...
  integer     :: jx                      ! Counter for the loop on 
                                         !   cartesian directions
  integer     :: ildauso                 ! Counter for loop on 4 center integral
  ...
  integer     :: ldauidx                 ! Index of the LDA+U projector
  integer     :: kua                     ! Equivalent atom in the unit cell
  integer     :: ks                      ! Kind of atomic species for atom kua
  integer     :: nldauproj               ! Number of LDA+U projectors for atom
                                         !  kua
  real(dp)    :: rmaxo                   ! Maximum range of the atomic orbitals 
  real(dp)    :: rmaxldau                ! Maximum range of the lda+U projectors
  ...
  complex(dp) ::  H_ldau_so_dc(maxnh,4)
                                         ...
                                         !   Hamiltonian matrix elements
  integer, save :: max_number_ldau_proj = 0
...
!   Find maximum number of LDA+U projectors on a given atom
    max_number_ldau_proj = 0
    do ka = 1, na_u
      is  = isa(ka)
      spp => species(is)
      max_number_ldau_proj = max(max_number_ldau_proj,spp%nprojsldau)
    enddo
...
! Make list of all orbitals needed for this node
 ...
 &               name='listedall', routine='ldau_so_hamil_2' )
...
! with a LDA+U projector
 ...
 &               name='iano', routine='ldau_so_hamil_2' )
  call re_alloc( iono, 1, maxno,                                  &
 &               name='iono', routine='ldau_so_hamil_2' )
  call re_alloc( xno, 1, 3, 1, maxno,                             &
 &               name='xno', routine='ldau_so_hamil_2' )
  call re_alloc( Ski, 1, max_number_ldau_proj, 1, maxno,          &
 &               name='Ski', routine='ldau_so_hamil_2' )
  call re_alloc( grSki, 1, 3, 1, max_number_ldau_proj, 1, maxno,  &
 &               name='grSki', routine='ldau_so_hamil_2' )
...
! Initialize arrays Di and listed only once
 ...
 &               name='Di', routine='ldau_so_hamil_2' )
  Di = 0.0_dp

! Initialize arrays Vi_Hubbard, Vi_dc, and listed only once
  call re_alloc( Vi_Hubbard, 1, no_s, 1, spin%grid,         &
 &               name='Vi_Hubbard', routine='ldau_so_hamil_2' )
  Vi_Hubbard = cmplx(0.0_dp, 0.0_dp, kind = dp)

  call re_alloc( Vi_dc, 1, no_s, 1, spin%grid,              &
 &               name='Vi_dc', routine='ldau_so_hamil_2' )
  Vi_dc = cmplx(0.0_dp, 0.0_dp, kind = dp)

  call re_alloc( listed, 1, no_s,                           &
 &               name='listed', routine='ldau_so_hamil_2')
  listed = .false.

!! For debugging
!  write(6,'(a,i5)')                                           &
! &  'ldau_so_hamil_2: nspin       = ' ,                       &
! &                    spin%dm
!  write(6,'(a,i5)')                                           &
! &  'ldau_so_hamil_2: nspin DM    = ' ,                       &
! &                    spin%DM  
!  write(6,'(a,i5)')                                           &
! &  'ldau_so_hamil_2: nspin H     = ' ,                       &
! &                    spin%H   
!  write(6,'(a,i5)')                                           &
! &  'ldau_so_hamil_2: nspin Grid  = ' ,                       &
! &                    spin%grid   
!  write(6,'(a,i5)')                                           &
! &  'ldau_so_hamil_2: nspin Spinor= ' ,                       &
! &                    spin%spinor 
!  write(6,'(a,i5)')                                           &
! &  'ldau_so_hamil_2: maximum number of LDA+U projectors = ', &
! &                    max_number_ldau_proj
!  write(6,'(a,2i5,3f12.5)')                                   &
! &  'ldau_so_hamil_2: Node, Nodes, rmaxo, rmaxldau, rmax = ', &
! &                    Node, Nodes, rmaxo, rmaxldau, rmax 
!  call die()
!! End debugging
...
!!   For debugging
!    write(6,'(a,2i5,f12.5)')                              &
! &    'ldau_so_hamil_2: Node, Nodes, E_correc_dc = ',     &
...
!         any of the LDA+U projectors of the atom ka
          do mproj = 1, nldauproj
!           Find the index of the LDA+U projector
            ldauidx = spp%pjldau_index(mproj)
...
!         Check maxno - if too small then increase array sizes
     ...
     &                       copy=.true., routine='ldau_so_hamil_2' )
              call re_alloc( iono, 1, maxno, name='iono',                      &
     &                       copy=.true., routine='ldau_so_hamil_2' )
              call re_alloc( xno, 1, 3, 1, maxno, name='xno',                  &
     &                       copy=.true., routine='ldau_so_hamil_2' )
              call re_alloc( Ski,1, max_number_ldau_proj, 1, maxno, name='Ski',&
     &                       copy=.true., routine='ldau_so_hamil_2' )
              call re_alloc( grSki, 1, 3, 1, max_number_ldau_proj, 1, maxno,   &
     &                       name='grSki', routine='ldau_so_hamil_2',          &
     &                       copy=.true. )
...
! &          'ldau_so_hamil_2: Node, Nodes, io, jo, Dscf_cmplx 1 = ',       &
! &            Node, Nodes, io, jo, Dscf_cmplx_1
!          write(6,'(a,4i5,2f12.5)')                                        &
! &          'ldau_so_hamil_2: Node, Nodes, io, jo, Dscf_cmplx 2 = ',       &
! &            Node, Nodes, io, jo, Dscf_cmplx_2
!          write(6,'(a,4i5,2f12.5)')                                        &
! &          'ldau_so_hamil_2: Node, Nodes, io, jo, Dscf_cmplx 3 = ',       &
! &            Node, Nodes, io, jo, Dscf_cmplx_3
!          write(6,'(a,4i5,2f12.5)')                                        &
! &          'ldau_so_hamil_2: Node, Nodes, io, jo, Dscf_cmplx 4 = ',       &
...
!           are stored
            ldauintegrals => spp%ldau_so_integrals(spp%pjldau_index(mproj))

!           Set the values for U and J for this correlated shell
            U = spp%pjldaunl_U(spp%pjldau_index(mproj))
            J = spp%pjldaunl_J(spp%pjldau_index(mproj))
...
!               Third loop on LDA+U projectors of atom ka
                do mproj2prime = 1, nldauproj

!                 Fourth loop on LDA+U projectors of atom ka
                  do mproj3prime = 1, nldauproj

                    direct_int =                                               &
 &                    ldauintegrals%vee_4center_integrals( mproj,              &
 &                                                         mprojprime,         &
 &                                                         mproj2prime,        & 
 &                                                         mproj3prime  )
                    fock_int   =                                               &
...
!     Restore Di and listed
 ...
 &    1.0_dp * ( real( H_ldau_so_dc(ind,1)*conjg(Dscf_cmplx_1), dp)        +  &
 &               real( H_ldau_so_dc(ind,2)*conjg(Dscf_cmplx_2), dp)        +  &
 &               real( H_ldau_so_dc(ind,4)*conjg(Dscf_cmplx_4), dp)        +  &
 &               real( H_ldau_so_dc(ind,3)*conjg(Dscf_cmplx_3), dp) )
  enddo 
...
!      jo = listh(ind)
!      do ispin = 1, spin%grid
!        if( abs(real(H_ldau_so(ind,ispin))) .gt. 1.d-4 .or.               &
! &          abs(aimag(H_ldau_so(ind,ispin))) .gt. 1.d-4 )                 &
! &      write(6,'(a,6i5,2f12.5)')                                         &
! &      'ldau_so_hamil_2: Node, Nodes, io, ind, jo, ispin, H_ldau_so = ', &
! &       Node, Nodes, io_local, ind, jo, ispin, H_ldau_so(ind,ispin)
!      enddo 
!    enddo 
!  enddo 
!
!   write(6,'(a,2i5,f12.5)')                           &
! &   'ldau_so_hamil_2: Node, Nodes, E_ldau_so    = ', &
! &                     Node, Nodes, E_ldau_so * 13.6058_dp
!
!   write(6,'(a,2i5,f12.5)')                           &
! &   'ldau_so_hamil_2: Node, Nodes, E_correc_dc  = ', &
! &                     Node, Nodes, E_correc_dc * 13.6058_dp
...
! &     'ldau_so_hamil_2: Node, Nodes, ia, fa = ',      &
! &     Node, Nodes, ia, fa(:,ia)
!   enddo 
!  call die()
!! End debugging
...
! Stop time counter
  call timer( 'ldau_so_hamil_2', 2 )

end subroutine ldau_so_hamil_2

end module m_pot_ldau
