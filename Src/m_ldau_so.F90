...
module m_ldau_so
  ...
  use sparse_matrices, only : maxnh         ! Number of non-zero elements in 
                                            !   the sparse matrix 
                                            !   (local to MPI Node)
  use m_occ_proj,      only : occ_proj_ldau ! Subroutine to compute the
                                            !   occupations of the LDA+U 
                                            !   projectors
  use m_pot_ldau,      only : ldau_so_hamil_2 ! Subroutine to compute the
  ...
  subroutine ldau_so_hamil( H_ldau_so, fal, stressl )

     complex(dp), intent(inout) :: H_ldau_so(maxnh,4)  ! Local copy of 
                                                       !    Hamiltonian matrix
                                                       !    elements related 
                                                       !    with the LDA+U 
     ...
     call occ_proj_ldau
     call ldau_so_hamil_2( H_ldau_so, fal, stressl )
