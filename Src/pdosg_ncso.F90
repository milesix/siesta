...
!
subroutine pdosg_ncso( nuo, no, maxuo, maxnh, nspin, &
  ...
  ! Condensed NC and SO versions, A. Garcia, 2023
  ! ****  INPUT  *********************************************************
  ...
  !                             with any orbital
  ! integer nspin             : 4 or 8, for NC and SO, respectively
  ...
  integer :: nuo, no, maxuo, maxnh, nhist, nuotot, nspin

  integer :: numh(nuo), listhptr(nuo), listh(maxnh), indxuo(no)

  real(dp), target :: H(maxnh,nspin)
  real(dp) :: S(maxnh), E1, E2, sigma, eo(nuotot*2),  &
...
!!$OMP end parallel
  ...
  call re_alloc( aux_red, 1, 4, 1, nuotot, 1, nhist, &
      name='aux_red', routine='pdosg_ncso')
  ...
  call de_alloc(aux_red, name='aux_red', routine='pdosg_ncso')
...
contains
  ...
  subroutine setup_Gamma()
    ...
    real(dp), pointer, dimension(:) :: h5, h6, h7, h8
    real(dp), allocatable, target   :: zero(:)

    allocate(zero(size(H,dim=1)))
    zero(:) = 0.0_dp
    ...
    do io = 1,nuo
      ...
      do j = 1,numh(io)
        ...
        Haux(1,jo,1,io) = Haux(1,jo,1,io) + cmplx(H(ind,1), h5(ind),dp)
        Haux(2,jo,2,io) = Haux(2,jo,2,io) + cmplx(H(ind,2), h6(ind),dp)
        Haux(1,jo,2,io) = Haux(1,jo,2,io) + cmplx(H(ind,3),-H(ind,4),dp)
        Haux(2,jo,1,io) = Haux(2,jo,1,io) + cmplx(H7(ind), H8(ind),dp)
      end do
    end do
