...
!     
      SUBROUTINE MATEL( OPERAT, IS1, IS2, FUNC1, FUNC2, IO1, IO2, R12,
     .     S12, DSDR )
C     *******************************************************************
...
C     LABEL1         : Label of 1st orbital
C     LABEL2         : Label of 2nd orbital 
...
C     
C     INTEGER FUNCTION LOFIO(IS,LABEL,IO)
...
C     Modules -----------------------------------------------------------
      ...
      USE ALLOC
      USE ATMFUNCS, ONLY: LOFIO, PHIATM, RCUT, xphiatm, yphiatm, zphiatm
     .     ,get_func_type,func_t
      use m_recipes, only: spline, splint, derf
      use spher_harm
      use spher_harm2, only: ylmexp, ylmylm
      use m_radfft
C     -------------------------------------------------------------------
...
C     Internal variable types and dimensions ----------------------------
     ...
     .     JO1, JO2, JR,
     .     L, L1, L2, L3, LMAX,
     .     NILM, NILM1, NILM2, NJLM1, NJLM2,IT1,IT2, IT
      INTEGER, SAVE ::
...
c     Find the integer associated with the functions type
      IT1=get_func_type(func1)
      IT2=get_func_type(func2)
...
C     Find radial expansion of each orbital -----------------------------
      DO I = 1,2
         IF (I .EQ. 1) THEN
            ...
            IO = IO1
            IT = IT1
            FOUND = (INDF(IS,IT,IO,1) .NE. 0)
         ELSE
            IS = IS2
            FUNC = FUNC2
            IO = IO2
            IT = IT2
            FOUND = (INDF(IS,IT,IO,IOPER) .NE. 0)
         ENDIF
...
C     two orbitals
            ...
            RMAX = NR * DR
            DRTAB = RMAX / NRTAB
            IF ( RCUT(IS,FUNC,IO) .GT. RMAX ) THEN
...
C     Expand orbital in spherical harmonics
            IF ((I.EQ.1) .OR. (IOPER.LE.3)) THEN
               CALL YLMEXP( L, RLYLM, PHIATM, IS,  FUNC, IO, 0, NQ, 
     .              RMAX, NILM, ILM(NF+1:), F(0:,NF+1:) )
               INDF(IS,IT,IO,1) = NF+1
               INDF(IS,IT,IO,2) = NF+1
               INDF(IS,IT,IO,3) = NF+1
               NLM(IS,IT,IO,1) = NILM
               NLM(IS,IT,IO,2) = NILM
               NLM(IS,IT,IO,3) = NILM
            ELSE
...
C     Find radial expansion of overlap ----------------------------------
      IF1 = INDF(IS1,IT1,IO1,1)
      IF2 = INDF(IS2,IT2,IO2,IOPER)

      IF ( INDFF(IF1,IF2,IOPER) .EQ. 0 ) THEN
*     CALL TIMER( 'MATEL2', 1 )

         NILM1 = NLM(IS1,IT1,IO1,1)
         NILM2 = NLM(IS2,IT2,IO2,IOPER)
         DO IFLM1 = IF1,IF1+NILM1-1
            DO IFLM2 = IF2,IF2+NILM2-1

C     Check interaction range
               IF (RCUT(IS1,FUNC1,IO1)+RCUT(IS2,FUNC2,IO2) .GT.
     $              RMAX) THEN
                  call die('MATEL: NQ too small for required cutoff.')
               ENDIF
...
C     Return to real space
                  L = 0
                  CALL RADFFT( L, NQ, NQ*PI/RMAX, FQ(0:NQ,1), 
     $                 FR(0:NQ,1) )
                  CALL RADFFT( L, NQ, NQ*PI/RMAX, FQ(0:NQ,2), 
     $                 FR(0:NQ,2) )
                  CALL RADFFT( L, NQ, NQ*PI/RMAX, VQ(0:NQ,1), 
     $                 VR(0:NQ,1) )
                  CALL RADFFT( L, NQ, NQ*PI/RMAX, VQ(0:NQ,2), 
     $                 VR(0:NQ,2) )
...
C     Select NRTAB out of NR points
     ...
     .                 CALL DIE('matel ERROR: NQ must be multiple of
     $                 NRTAB')
                  DO IR = 0,NRTAB
                     JR = IR * NR / NRTAB
                     FFL(IR) = FFL(JR)
...
C     Find if radial function is already in table
                  FOUND = .FALSE.
                  SEARCH: DO JO1 = LBOUND(INDF,3),UBOUND(INDF,3)
                  DO JO2 = LBOUND(INDF,3),UBOUND(INDF,3)
                     JF1 = INDF(IS1,IT1,JO1,1)
                     JF2 = INDF(IS2,IT2,JO2,IOPER)
                     IF (JF1.NE.0 .AND. JF2.NE.0) THEN
                        NJLM1 = NLM(IS1,IT1,JO1,1)
                        NJLM2 = NLM(IS2,IT2,JO2,IOPER)
...
C     Store new radial function
               IF (.NOT.FOUND) THEN
                  ...
                  IF (NFFR .GT. MFFR) THEN
                     MFFR = EXPAND * NFFR
                     CALL RE_ALLOC( FFR, 0,NRTAB, 1,2, 1,MFFR,
     $                    MYNAME//'FFR')
                  ENDIF
                  IFFR(L3) = NFFR
                  CFFR(L3) = 1._dp
...
C     Expand the product of two spherical harmonics (SH) also in SH
            CALL YLMEXP( L1+L2, RLYLM, YLMYLM, ILM(IFLM1),func,
     .           ILM(IFLM2), 1, 1, 1.0_dp, NILM, ILMFF(NFFY+1:),
     .           FFY(NFFY+1:))
...
C     Find if orbitals are far (out of range)
      IF (R .GT. RCUT(IS1,FUNC1,IO1)+RCUT(IS2,FUNC2,IO2) ) THEN
         FAR = .TRUE.
         IF (OPERAT.EQ.'U') THEN
            IF1 = INDF(IS1,IT1,IO1,1)
            IF2 = INDF(IS2,IT2,IO2,IOPER)
