...
!
subroutine pdos2k( nuo, no, maxuo, maxnh, &
...
!!$OMP end parallel
  ...
  call de_alloc(aux_red, name='aux_red', routine='pdos2k')
  ...
  subroutine setup_k(k)
    ...
    do iuo = 1,nuo
      do j = 1,numh(iuo)
        ...
        Haux(1,juo,1,iuo) = Haux(1,juo,1,iuo) + cmplx(H(ind,1), 0._dp,dp) * kphs
        Haux(2,juo,2,iuo) = Haux(2,juo,2,iuo) + cmplx(H(ind,2), 0._dp,dp) * kphs
        Haux(1,juo,2,iuo) = Haux(1,juo,2,iuo) + cmplx(H(ind,3), - H(ind,4),dp) * kphs
        Haux(2,juo,1,iuo) = Haux(2,juo,1,iuo) + cmplx(H(ind,3), + H(ind,4),dp) * kphs
