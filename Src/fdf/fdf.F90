...
MODULE fdf
  ...
  interface
     function unit_conv_fac(from,to,stat,msg) result (factor)
       import :: dp
       character(len=*), intent(in)   :: from, to
       integer, intent(out)           :: stat
       character(len=96), intent(out) :: msg
       real(dp)                       :: factor
     end function unit_conv_fac
  end interface

#ifdef F2008
  procedure(unit_conv_fac), pointer :: internal_convfac => no_unit_conversion
#else
  procedure(unit_conv_fac), pointer :: internal_convfac => null()
#endif
...
! Match, search over blocks, and destroy block structure
  ...
  ! it is only usefulafter having set the
  ! unit conversion function
  public :: fdf_convfac
  public :: fdf_set_unit_conversion_function
...
!--------------------------------------------------------------------------- END
    ...
    ! Units processing
    !
    subroutine fdf_set_unit_conversion_function(func)
      procedure(unit_conv_fac) :: func

      internal_convfac => func
      
    end subroutine fdf_set_unit_conversion_function

    function no_unit_conversion(from,to,stat,msg) result (factor)
        character(len=*), intent(in)   :: from, to
        integer, intent(out)           :: stat
        character(len=96), intent(out) :: msg
        real(dp)                       :: factor
      ...
      end function no_unit_conversion
