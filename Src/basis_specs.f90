module basis_specs
  ...
  character(len=1), parameter   :: sym(0:4) = (/ 's','p','d','f','g' /)
...
CONTAINS
  ...
  subroutine read_basis_specs
    ...
    !------------------------------------------------------------------------
    reparametrize_pseudos = fdf_boolean('ReparametrizePseudos',.false.)
    ...
    !
    restricted_grid = fdf_boolean("Restricted.Radial.Grid", &
    ...
    if (synthetic_atoms) then
       ...
       loop: DO
          ...
          ns_read = ns_read + 1
          p => digest(line)
          if (.not. match(p,"i"))  call die("Wrong format in SyntheticAtoms")
          isp = integers(p,1)
          if (isp .gt. nspecies .or. isp .lt. 1) call die("Wrong specnum in SyntheticAtoms")
          basp=>basis_parameters(isp)
          ...
          p => digest(line)
          nns = nintegers(p)
          if (nns .lt. 4) call die("Please give all valence n's in SyntheticAtoms block")
          gs%n = 0
          do i = 1, nns
             gs%n(i-1) = integers(p,i)
          enddo
          call destroy(p)
          if (.not. fdf_bline(bp,line)) call die("No occupation info")
          p => digest(line)
          noccs = nvalues(p)
          ...
          do l=0,3
             if (gs%occupied(l)) write(6,'(2x,i1,a1,f8.5)',advance='no') &
    ...
    do isp=1,nspecies
       ...
       if (basp%synthetic) then
          gs => basp%ground_state
          if (gs%z_valence .lt. 0.001) call die("Synthetic species not detailed")
       endif
  ...
  subroutine ground_state(z,gs)
    ...
    do l=0,3
       ...
       if (gs%occupied(l)) write(6,'(2x,i1,a1,i2.2)',advance='no') &
  ...
  subroutine readkb
    ...
    loop: DO     !! over species
       if (.not. fdf_bline(bp,line)) exit loop
       p => digest(line)
       if (.not. match(p,"ni")) call die("Wrong format in PS.KBprojectors")
       isp = label2species(names(p,1))
       if (isp .eq. 0) then
          write(6,'(a,1x,a)')  "WRONG species symbol in PS.KBprojectors:",trim(names(p,1))
          call die
       endif
       basp=>basis_parameters(isp)
       ...
       do ish=1,basp%nkbshells
          ...
          if (.not. fdf_bline(bp,line)) then
             ...
             if (match(p,"ni")) then
                ...
                if (ish .ne. basp%nkbshells)call die("Not enough shells for this species...")
                call backspace(bp)
                call destroy(p)
             else
                ! would set erefs
...
2000 CONTINUE
    ...
    do isp=1, nspecies
       ...
       if (basp%floating) then
          if (basp%lmxkb .ne. -1)call die("Floating orbs cannot have KB projectors...")
       endif
    enddo
       ...
       do ish=1,basp%nkbshells
          ...
          if (.not. fdf_bline(bp,line)) then
             ...
             if (match(p,"ni")) then
                ...
                if (nvalues(p) .ne. k%nkbl)call die("Wrong number of energies")
                unitstr = defunit
                if (nnames(p) .eq. 1) unitstr = names(p,1)
                ! Insert ref energies in erefkb
                do i=1,k%nkbl
                   k%erefKB(i) = values(p,i)*fdf_convfac(unitstr,defunit)
                enddo
                call destroy(p)
             endif
          endif
  ...
  subroutine repaobasis
    ...
    write(6,'(/,a)') "Repaobasis: processing %block PAO.Basis"

    loop: DO !! over species

       if (.not. fdf_bline(bp,line)) exit loop
       p => digest(line)
       if (.not. match(p,"ni")) call die("Wrong format in PAO.Basis")
       isp = label2species(names(p,1))
       if (isp .eq. 0) then
          write(6,'(a,1x,a)') "WRONG species symbol in PAO.Basis:",trim(names(p,1))
          call die
       endif
       ...
       write(6,'(a,a)') "Repaobasis: species: ",basp%label
       write(6,'(a,i2)') "Repaobasis:  Number of shells=",basp%nshells_tmp
       ...
       shells: do ish=1,basp%nshells_tmp
          ...
          write(6,'(a,2i2)') "Repaobasis:     Shell with n,l=",s%n,s%l
          ! Optional stuff: Polarization, Soft-confinement Potential

          if (search(p,"S",index_splnorm)) then
             if (match(p,"v",after=index_splnorm)) then
                s%split_norm = values(p,ind=1,after=index_splnorm)
                if (s%split_norm == 0.0_dp) write(6,"(a)") &
          ...
          allocate(s%rc(1:s%nzeta),s%lambda(1:s%nzeta),s%orb(1:s%nzeta))
          allocate(s%energies(1:s%nzeta),s%spln(1:s%nzeta),s%rphi(1:s%nzeta))
          allocate(s%population(1:s%nzeta))
          ...
          if (.not. fdf_bline(bp,line)) then
             if (ish.ne.basp%nshells_tmp)call die("Not enough shells")
             ...
             if (.not.match(p,"r")) then
                ...
                if (nreals(p).ne.s%nzeta) call die("Wrong number of lambdas")
                do i=1,s%nzeta
    ...
    do isp = 1, nspecies
       ...
       loop_l: do l=0,basp%lmxo
          ...
          do ish=1, basp%nshells_tmp
             ...
             if (s%l .eq. l) nn=nn+1
          enddo
          if (ls%nn == 0) ls%nn = nn !The shell can be a pol shell
          if (ls%nn.eq.0) then
             !! What else do we do here?
             cycle loop_l  
          endif
          !! 
          if (.not. associated(ls%shell)) allocate(ls%shell(1:nn))
          ...
          do ish=1, basp%nshells_tmp
             ...
             if (s%l .eq. l) then
                ...
                if(s%polarized)then !Fill the polarization shell
                   ...
                   basp%lshell(s%l_shell_polarizes)%nn = 1
                   basp%lshell(s%l_shell_polarizes)%l=s%l_shell_polarizes
                   spol=>basp%lshell(s%l_shell_polarizes)%shell(1)                                  
                   spol%l_shell_polarized = s%l
                   spol%l                 = s%l+1
                   ...
                   spol%rc                = 0.0_dp
                   spol%lambda            = 1.0_dp
                   spol%split_norm        = s%split_norm
                   spol%rinn              = s%rinn
                   spol%vcte              = s%vcte
                   allocate(spol%orb(1:spol%nzeta),spol%rphi(1:spol%nzeta))
                   allocate(spol%energies(1:spol%nzeta),spol%population(1:spol%nzeta))
                endif
             endif
          enddo
          if (nn.eq.1) then
  ...
  function set_default_lmxkb(is) result(lmxkb)
    ...
    if (basp%floating) return   ! Leave it at -1 for floating orbs.
    ...
    do l = 0, basp%lmxo
       ...
       do i = 1, ls%nn
          s=>ls%shell(i)
          if (s%polarizes) lpol = s%l !+ 1
       enddo
    enddo

    write(6,'(3a,i1,/,2a,/,a)') 'For ', trim(basp%label), &
         ', standard SIESTA heuristics set lmxkb to ', lmxkb, &
  ...
  subroutine resizes
    ...
    ! Reading atomic basis sizes for different species.
    !
    ! Reads fdf block. Not necessarily all species have to be given. The
    ! ones not given at input will be assumed to have the basis sizes
    ! given by the general input PAO.BasisSize, or its default value.
    ...
    loop: DO
       if (.not.fdf_bline(bp,line)) exit loop
       p => digest(line)
       if (.not. match(p,"nn"))call die("Wrong format in PAO.BasisSizes")
       isp = label2species(names(p,1))
       if (isp .eq. 0) then
          write(6,'(a,1x,a)')"WRONG species symbol in PAO.BasisSizes:",&
               trim(names(p,1))
          call die
       else
          basp=>basis_parameters(isp)
          ...
          write(6,'(4a)') 'resizes: Read basis size for species ',&
  ...
  subroutine relmxkb
    ! Reads the maximum angular momentum of the Kleinman-Bylander
    ! projectors for the different species.
    !
    ! Reads fdf block. Not necessarily all species have to be given.
    ...
    loop: DO
       ...
       if (isp .eq. 0) then
          ...
          write(6,"(a, i4, 2a)") &
               'relmxkb: Read Max KB Ang. Momentum= ',basp%lmxkb_requested, &
  ...
  subroutine remass
    ...
    loop: DO
       if (.not. fdf_bline(bp,line)) exit loop
       p => digest(line)
       if (.not. match(p,"iv")) call die("Wrong format in AtomicMass")
       isp = integers(p,1)
       if (isp .gt. nspecies .or. isp .lt. 1)  call die("Wrong specnum in AtomicMass")
       basp=>basis_parameters(isp)
       basp%mass = values(p,2)
       write(6,"(a, i4, a, f12.5)") &
            'remass: Read atomic mass for species ', isp,' as ', basp%mass
       call destroy(p)
    enddo loop
    call destroy(bp)
  ...
  subroutine semicore_check(is)
    ...
    if(abs(ndiff-abs(Zval-zval_vps)).gt.tiny) then
       write(6,'(2a)') 'ERROR expected valence charge for species ', &
            basp%label
       write(6,'(a)') 'ERROR and the value read from the vps file'
  ...
  subroutine autobasis
    ...
    loop: do isp=1, nspecies
       ...
       if (basp%lmxo .ne. -1) cycle loop !Species already in PAO.Basis block
       if (basp%semic) then
          write(6,'(2a)') basp%label,' must be in PAO.Basis (it has semicore states)'
          call die
       endif

       if (basp%bessel) then
          write(6,'(2a)') basp%label,' must be in PAO.Basis (it is a floating Bessel function)'
          call die
       endif

       !Set the default l max  
       basp%lmxo = basp%ground_state%lmax_valence

       !Reset to the smallest values
       lmax=0
       nmax=1
       ...
       !Polarization orbitals automatic setup
       if (basp%basis_size(3:3) .eq. 'p')then
          !    Setup of pol orbs. The automatic polarization scheme
          !    follows this rule:
          !     Orb    Pol orb.
          !      1s -> 2p
          !      2s -> 2p
          ...
          do i=0,4
             if (basp%ground_state%occupied(i))then
                lastnmax=nmax
                lastlmax=lmax
                nmax=max(basp%ground_state%n(i),nmax)
                lmax=i
             endif
          enddo
       ...
       do l=0, basp%lmxo
          ...
          !semicore? Not possible in autobasis
          !if there is semicore the user must provide a block PAO.Basis
          allocate(ls%shell(ls%nn))

          s => ls%shell(1)
          s%l = l
          s%n = basp%ground_state%n(l)
          s%polarizes = .false.
          s%z_valence = basp%ground_state%z_valence
          ...
          if (s%nzeta .ne.0) then
             allocate(s%orb(1:s%nzeta))
             allocate(s%rc(1:s%nzeta))
             allocate(s%lambda(1:s%nzeta))
             allocate(s%energies(1:s%nzeta),s%rphi(1:s%nzeta)) 
             allocate(s%spln(1:s%nzeta),s%population(1:s%nzeta))
